...
import { PROGRESS_NOTIFICATION_ID } from '../const/immutables';
...
const initialState = {
  ...
  reportModalProjects: [],
  progressNotification: { show: null, id: PROGRESS_NOTIFICATION_ID },
  hoverActive: false
};
...
export const importProgressSlice = createSlice({
  ...
  reducers: {
    ...
    getQuickImportResultFulfilled: (state, { payload }) => {
      if (!payload?.fromEmail) state.loader.report = false;
