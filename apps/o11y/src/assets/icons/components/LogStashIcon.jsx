import React from 'react';

const LogStashIcon = (props) => (
  <svg
    width="48"
    height="48"
    viewBox="0 0 48 48"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <rect x="8" y="8" width="32" height="32" fill="url(#patternLS)" />
    <defs>
      <pattern
        id="patternLS"
        patternContentUnits="objectBoundingBox"
        width="1"
        height="1"
      >
        <use xlinkHref="#image0_3357_313791" transform="scale(0.03125)" />
      </pattern>
      <image
        id="image0_3357_313791"
        width="32"
        height="32"
        xlinkHref="data:image/png;base64,"
      />
    </defs>
  </svg>
);

export default LogStashIcon;
