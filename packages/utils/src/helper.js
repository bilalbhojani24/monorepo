...
export const getInitialAndFinalFocusableElement = (element) => {
  const querySelectorToTargetFocusableElements = "input, button, [role='button'], [role='link'], a, select, textarea";
  if (!element) {
    return [null];
  }
  const focusableElements = [...element.querySelectorAll(querySelectorToTargetFocusableElements)];
  return focusableElements.length ? [focusableElements.shift(), focusableElements.pop()] : [null];
};

export const trapFocusInElement = (event, parentElement, firstElement = null, lastElement = null) => {
  /*
  ...
  if (!firstElement || !lastElement) {
    [firstFocusElement, lastFocusedElement] = getInitialAndFinalFocusableElement(parentElement);
  }
...
export const utmDataMap = (queryString) => {
  ...
  Object.keys(utmDataObj).forEach((key) => utmDataObj[key] === undefined && delete utmDataObj[key]);
  return utmDataObj;
};
 ...
 */
export const webEventTracker = (eventName, eventObj = {}, skipLoggingKeys = [], cb = null, sendToGA = false) => {
  if (window.WebEventTracker && window.EDS) {
    window.WebEventTracker.logEvent(skipLoggingKeys, window.EDS.webEvents, eventName, eventObj, cb, sendToGA);
  }
};
