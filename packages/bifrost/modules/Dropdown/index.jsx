...
const Dropdown = (props) => {
  const { options, dividerRequired, triggerTitle, triggerVariant, headerRequired, heading, subHeading } = props;

  return (
    <Menu as="div" className="relative inline-block text-left">
      <div>
        {triggerVariant === 'text-button' && (
          <Menu.Button className="inline-flex w-full justify-center rounded-md border border-base-300 bg-white px-4 py-2 text-sm font-medium text-base-700 shadow-sm hover:bg-base-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-base-100">
            {triggerTitle}
        ...
        {['meatball-button', 'menu-button'].includes(triggerVariant) && (
          <Menu.Button
            className={classNames(
              'flex items-center rounded-full bg-base-100 text-base-400 hover:text-base-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-base-100 p-2',
              {
        ...
        <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-base-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          {headerRequired && (
            <div className="px-4 py-3">
              ...
              <p className="truncate text-sm font-medium text-base-900">{subHeading}</p>
            </div>
          )}
          <div className="py-1">
                    ...
                    className={classNames(
                      {
                        ...
                        'border-b border-base-100': dividerRequired && optionIdx !== options.length - 1
                      },
