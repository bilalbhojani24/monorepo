...
const checkPresentVariable = (productEnv, val) =>
  variables[productEnv] && variables[productEnv][val]
    ? variables[productEnv][val]
    : false;
...
const testAgainstString = (prop, value, source, mappedFiles, comparison) => {
  ...
  const selectedEnv = Object.keys(mappedFiles).find((key) =>
    mappedFiles[key]
      ? source.input.file.match(mappedFiles[key].filePath)
      : false
  );
  const productEnv = selectedEnv || "common";
  const isAccepted = mappedFiles[selectedEnv]
    ? !!source.input.file.match(mappedFiles[selectedEnv].styles)
    : true;
  ...
  if (isStringRegex(comparison)) {
    return {
      productEnv,
      isProp: new RegExp(comparison.slice(1, -1)).test(prop),
    };
  }
...
const checkProp = (prop, value, source, mappedFiles, targets) => {
  ...
  targets.forEach((target) => {
    const { productEnv, isProp } = testAgainstString(
      prop,
      value,
      source,
      mappedFiles,
...
module.exports = stylelint.createPlugin(
  ruleName,
  (properties, propertyConfigs, context) => {
    ...
    return (postcssRoot, result) => {
      ...
      postcssRoot.walkDecls((statement) => {
        ...
        if (
          statement.prop === "font-size" &&
          variables.trike.font[statement.value]
        ) {
          associatedVariable = variables.trike.font[statement.value];
        } else if (
          statement.prop.match("color") &&
          variables.trike.color[statement.value]
        ) {
        ...
        if (
          ...
          associatedVariable = checkPresentVariable(
            productEnv,
            statement.value
          );
        }
